...
static uint32_t *waveform[2] = { fullsintable, halfsintable };
...
static unsigned char opll_default_inst[OPLL_TONE_NUM][(16 + 3) * 8] = {
...
/* VRC7 VOICE */
static unsigned char vrc7_default_inst[OPLL_TONE_NUM][(16 + 3) * 8] = {
...
makeAdjustTable (void)
{
  ...
  for (i = 1; i < (1<<EG_BITS); i++)
    AR_ADJUST_TABLE[i] = (uint32_t) ((double) (1<<EG_BITS)-1 - ((1<<EG_BITS)-1)*log(i)/log(127));
}
...
void
OPLL_getDefaultPatch (bool isVRC7, int32 type, int32 num, OPLL_PATCH * patch)
{
	if (isVRC7)
		OPLL_dump2patch (vrc7_default_inst[type] + num * 8, patch);
	else
		OPLL_dump2patch (opll_default_inst[type] + num * 8, patch);
}

static void
makeDefaultPatch (bool isVRC7)
{
  int32 i, j;

  for (i = 0; i < OPLL_TONE_NUM; i++)
    for (j = 0; j < 19; j++)
      OPLL_getDefaultPatch (isVRC7, i, j, &default_patch[i][j * 2]);
...
static void
maketables (uint32_t c, uint32_t r, bool isVRC7)
{
  if (c != clk)
  {
    clk = c;
...
OPLL *
OPLL_new (uint32_t _clk, uint32_t _rate, bool isVRC7)
{
  OPLL *opll;
  int32 i;

  maketables (_clk, _rate, isVRC7);

  opll = (OPLL *) calloc (sizeof (OPLL), 1);
  if (opll == NULL)
    return NULL;

  opll->vrc7_mode = isVRC7;

  for (i = 0; i < 19 * 2; i++)
    memcpy(&opll->patch[i],&null_patch,sizeof(OPLL_PATCH));
//  for (i = 0; i < 14; i++)
//    centrpanning( opll->pan[i] );
...
/* Update Noise unit */
static void
updatnoise (OPLL * opll)
{
   if(opll->noisseed&1) opll->noisseed ^= 0x8003020;
   opll->noisseed >>= 1;
}
...
*/
INLINE int32
calc_slot_hat (OPLL_SLOT *slot, int32 pgout_cym, uint32_t noise)
{
  uint32_t dbout;
 ...
 if (! opll->vrc7_mode)
 {
  ...
  else
  {
    if (!(opll->mask & OPLL_MASK_HH) && (MOD(opll,7)->eg_mode != FINISH))
      perc += calc_slot_hat (MOD(opll,7), CAR(opll,8)->pgout, opll->noisseed&1);
    if (!(opll->mask & OPLL_MASK_SD) && (CAR(opll,7)->eg_mode != FINISH))
      perc -= calc_slot_snare (CAR(opll,7), opll->noisseed&1);
  }
...
OPLL_calc (OPLL * opll)
{
  ...
  opll->out = (int32) (((double) opll->next * (opll->opllstep - opll->oplltime)
                          + (double) opll->prev * opll->oplltime) / opll->opllstep);

  return (int32) opll->out;
}
...
void UpdateAfterLoad(OPLL * opll) {
	updatrhythm_mode(opll);
	updatkey_status(opll);
	for (int i=0; i <9; i++) {
		UPDATE_ALL(MOD(opll,i));
		UPDATE_ALL(CAR(opll,i));
	}
}
