#include "s_MSM6585.hpp"

MSM6585::MSM6585 (int32_t newHostClock, int (*newGetInput)(void)): hostClock(newHostClock), getInput(newGetInput), lowPassFilter(2, newHostClock, 4000.0 *0.4) {
	reset();
	setRate(0);
}

void MSM6585::reset (void) {
	count = 0;
	step = 0;
	signal = -2;
}

void MSM6585::setRate (uint8_t rateByte) {
	rate = 4000 <<(rateByte &3);
	lowPassFilter.recalc(2, hostClock, rate *0.4);
	count = 0;
}
...
void MSM6585::run (void) {
	count += rate;
	while (count >= hostClock) {
		count -= hostClock;
		uint8_t nibble;		
		if (whichNibble)
			nibble = input &0x0F;
...
int MSM6585::getOutput (void) {
	return lowPassFilter.process(signal*16 +1e-15);
}

int MAPINT MSM6585::saveLoad (STATE_TYPE stateMode, int offset, unsigned char *stateData) {
	SAVELOAD_BOOL(stateMode, offset, stateData, whichNibble);
	SAVELOAD_BYTE(stateMode, offset, stateData, input);
	SAVELOAD_WORD(stateMode, offset, stateData, signal);
	SAVELOAD_LONG(stateMode, offset, stateData, count);
	SAVELOAD_LONG(stateMode, offset, stateData, hostClock);
	SAVELOAD_LONG(stateMode, offset, stateData, rate);
	SAVELOAD_WORD(stateMode, offset, stateData, step);
	return offset;
}
