...
#include "h_PIC16C5x.h"

PIC16C5X::PIC16C5X(int program_width, int data_width, int picmodel, uint8_t* rom, cbReadIO _readIO, cbWriteIO _writeIO):
	m_picmodel(picmodel),
	m_temp_config(0),
	m_read(_readIO),
	m_write(_writeIO),
...
PIC16C54::PIC16C54(uint8_t* rom, cbReadIO _readIO, cbWriteIO _writeIO): PIC16C5X( 9, 5, 0x16C54, rom, _readIO, _writeIO) {};
PIC16C55::PIC16C55(uint8_t* rom, cbReadIO _readIO, cbWriteIO _writeIO): PIC16C5X( 9, 5, 0x16C55, rom, _readIO, _writeIO) {};
PIC16C56::PIC16C56(uint8_t* rom, cbReadIO _readIO, cbWriteIO _writeIO): PIC16C5X(10, 5, 0x16C56, rom, _readIO, _writeIO) {};
PIC16C57::PIC16C57(uint8_t* rom, cbReadIO _readIO, cbWriteIO _writeIO): PIC16C5X(11, 7, 0x16C57, rom, _readIO, _writeIO) {};
PIC16C58::PIC16C58(uint8_t* rom, cbReadIO _readIO, cbWriteIO _writeIO): PIC16C5X(11, 7, 0x16C58, rom, _readIO, _writeIO) {};
PIC1650::PIC1650  (uint8_t* rom, cbReadIO _readIO, cbWriteIO _writeIO): PIC16C5X( 9, 5, 0x1650,  rom, _readIO, _writeIO) {};
PIC1655::PIC1655  (uint8_t* rom, cbReadIO _readIO, cbWriteIO _writeIO): PIC16C5X( 9, 5, 0x1655,  rom, _readIO, _writeIO) {};
...
{
	EMU->DbgOut(L"PIC16C5x:  PC=%03x,  Illegal opcode = %04x\n", (m_PC-1), m_opcode.s0);
}
...
void PIC16C5X::reset(RESET_TYPE reset_type)
{
...
{
	EMU->DbgOut(L"Writing %04x to the PIC16C5x config register\n", data);
	m_temp_config = data;
}
		...
		if (((old_WDT != 0) && (old_WDT < m_WDT)) || (m_WDT == 0))
		{
			if (PSA) {
				...
				if (m_prescaler >= (1 << PS)) { /* Prescale values from 1 to 128 */
					...
					CLR(STATUS, TO_FLAG);
					//PIC16C5x_soft_reset();
					EMU->DbgOut(L"Watchdog-triggered reset");
				}
			}
			else {
				CLR(STATUS, TO_FLAG);
...
void PIC16C5X::run()
{
	...
	while (m_icount > 0) {
		...
		else
		{
			...
			//EMU->DbgOut(L"%04X: %03X", m_PC, m_opcode.s0);
			m_PC++;
			PCL++;
...
int	PIC16C5X::saveLoad (STATE_TYPE stateMode, int offset, unsigned char *data) {
	SAVELOAD_WORD(stateMode, offset, data, m_PC);
	SAVELOAD_WORD(stateMode, offset, data, m_PREVPC);
	SAVELOAD_BYTE(stateMode, offset, data, m_W);
	SAVELOAD_BYTE(stateMode, offset, data, m_OPTION);
	SAVELOAD_WORD(stateMode, offset, data, m_CONFIG);
	SAVELOAD_BYTE(stateMode, offset, data, m_ALU);
	SAVELOAD_WORD(stateMode, offset, data, m_WDT);
	SAVELOAD_BYTE(stateMode, offset, data, m_TRISA);
	SAVELOAD_BYTE(stateMode, offset, data, m_TRISB);
	SAVELOAD_BYTE(stateMode, offset, data, m_TRISC);
	for (auto& c: m_STACK) SAVELOAD_WORD(stateMode, offset, data, c);
	SAVELOAD_WORD(stateMode, offset, data, m_prescaler);
	SAVELOAD_WORD(stateMode, offset, data, m_opcode.s0);
	for (auto& c: m_internalram) SAVELOAD_BYTE(stateMode, offset, data, c);
	SAVELOAD_LONG(stateMode, offset, data, m_icount);
	SAVELOAD_LONG(stateMode, offset, data, m_delay_timer);
	SAVELOAD_LONG(stateMode, offset, data, m_rtcc);
	SAVELOAD_BOOL(stateMode, offset, data, m_count_pending);
	SAVELOAD_LONG(stateMode, offset, data, m_inst_cycles);
	SAVELOAD_LONG(stateMode, offset, data, m_clock2cycle);
	return offset;
}