...
INT_PTR	CALLBACK	MoviePlayProc (HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDC_MOVIE_PLAY_BROWSE:
			...
			ofn.nMaxFileTitle = 0;
			ofn.lpstrInitialDir = Settings::Path_NMV;
			...
			_tcscpy_s(Settings::Path_NMV, filename);
			Settings::Path_NMV[ofn.nFileOffset-1] = 0;
			...
			_stprintf_s(str, _T("Length: %i:%02i.%02i"), Len / 3600, (Len % 3600) / 60, Len % 60);
...
/*	fread(buf, 4, 1, Data);	// to see if Game Genie is used or not
	...
	if (buf[3] & 0x40)		// we need to check this BEFORE reset so we can possibly swap in the menu
		Settings::GameGenie = TRUE;	// this bit ONLY gets set for reset-based movies
	else	Settings::GameGenie = FALSE;	// if it starts from a savestate, it's already in the game (and we load the GENI block)*/
	...
	NES::SetRegion((buf[3] & 0x80) ? Settings::REGION_PAL : Settings::REGION_NTSC);
...
void	Record (void)
{
	...
	if ((NES::CurRegion != Settings::REGION_NTSC) && (NES::CurRegion != Settings::REGION_PAL))
	{
	...
	x = PPU::PPU[0]->PALRatio ? 0x80 : 0x00;
	FrameLen = 0;
	...
	if (NES::HasMenu)
		FrameLen++;
	x |= FrameLen;
	if (!fromState)	// set Game Genie bit only if recording from reset
