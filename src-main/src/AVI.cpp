...
void	Start (void)
{
	...
	wfx.nAvgBytesPerSec = SAMPLING_RATE*2;
	wfx.nBlockAlign = 2;
	wfx.wBitsPerSample = 16;
	wfx.cbSize = 0;

	if (RI.ConsoleType ==CONSOLE_VT369 && reg4100[0x1F] &0x01) {
		if (NES::CurRegion == Settings::REGION_NTSC || NES::CurRegion == Settings::REGION_DENDY && Settings::Dendy60Hz)
			handle = CreateAvi(FileName, 33278526, &wfx);	// 60.098816 fps/2
		else	
			handle = CreateAvi(FileName, 39994418, &wfx);	// 50.006978 fps/2
	} else
	if (NES::CurRegion == Settings::REGION_NTSC || NES::CurRegion == Settings::REGION_DENDY && Settings::Dendy60Hz)
	...
	bmih.biWidth = GFX::SIZEX;
	bmih.biHeight = GFX::SIZEY;
	bmih.biPlanes = 1;
	bmih.biBitCount = 32;
...
void	AddVideo (void)
{
	...
	register unsigned short *src = PPU::PPU[0]->DrawArray +GFX::OFFSETX +GFX::OFFSETY*341;
	register unsigned long *dst = videoBuffer + GFX::SIZEX * GFX::SIZEY;
...
void	AddAudio (void)
{
	...
	hr = AddAviAudio(handle, Sound::buffer, Sound::LockSize);
	if (hr)
	{
		TCHAR msg[256];
		FormatAviMessage(hr, msg, 256);
