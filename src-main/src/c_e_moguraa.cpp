...
#include <poppack.h>
int	ExpPort_Moguraa::Save (FILE *out)
{
	int clen = 0;
...
int	ExpPort_Moguraa::Load (FILE *in, int version_id)
{
	int clen = 0;
	unsigned short len;
...
void	ExpPort_Moguraa::Frame (unsigned char mode)
{
	int i;
	if (mode & MOV_PLAY)
...
unsigned char	ExpPort_Moguraa::Read1 (void)
{
	return 0;
}
unsigned char	ExpPort_Moguraa::Read2 (void)
{
...
unsigned char	ExpPort_Moguraa::ReadIOP (uint8_t) {
	return 0;
}

void	ExpPort_Moguraa::Write (unsigned char Val)
{
	State->Bits = State->NewBits;
	State->Sel = ~Val & 7;
}
INT_PTR	CALLBACK	ExpPort_Moguraa_ConfigProc (HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
void	ExpPort_Moguraa::Config (HWND hWnd)
{
	DialogBoxParam(hInst, MAKEINTRESOURCE(IDD_MOGURAA), hWnd, ExpPort_Moguraa_ConfigProc, (LPARAM)this);
}
void	ExpPort_Moguraa::SetMasks (void)
{
}
ExpPort_Moguraa::~ExpPort_Moguraa (void)
{
	delete State;
	delete[] MovData;
}
ExpPort_Moguraa::ExpPort_Moguraa (DWORD *buttons)
{
	Type = EXP_MOGURAA;
	NumButtons = 12;
	Buttons = buttons;
	State = new ExpPort_Moguraa_State;
	MovLen = 2;
...
void ExpPort_Moguraa::CPUCycle (void) { }
} // namespace Controllers