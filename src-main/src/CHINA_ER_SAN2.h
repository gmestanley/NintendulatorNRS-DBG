BOOL g_bSan2;
unsigned char pSan2Font[64*1024];//定义64K的字库空间 Defining a 64K font space
unsigned char CPU_BACKUP[256];

#define RENDER_WIDTH 341
...
static unsigned char VRAM (unsigned short int Addr) {
	//Addr |= 0x2000;
	//return (EI.GetPPUReadHandlerDebug(Addr >>10))(Addr >>10, Addr &0x3FF);
	return PPU::PPU[0]->VRAM[Addr >>10][Addr &0x3FF];
}

void	CHINA_ER_SAN2_ShowFont(void)	{ //书写浮动的文本 Writing floating text 
					...
					for(int jj = 0; jj < 16; jj ++)
					{
						...
						if(pos2 < maxpos)
						{
							if(jj < 8 && (pSan2Font[index * 32 + ii * 2]&(0x80>>jj)))
								PPU::PPU[0]->DrawArray[pos2] = PPU::PPU[0]->Palette[color];
